...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create split-pay-1b19
  fly apps create split-pay-1b19-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app split-pay-1b19
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app split-pay-1b19-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'split-pay-1b19-staging' does not match app name in config file 'split-pay-1b19'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name split-pay-1b19-db
  fly postgres attach --app split-pay-1b19 split-pay-1b19-db

  fly postgres create --name split-pay-1b19-staging-db
  fly postgres attach --app split-pay-1b19-staging split-pay-1b19-staging-db
  ```
